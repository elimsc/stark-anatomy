...
def test_distributivity():
    ...
    lhs = a * (b + c)
    rhs = a * b + a * c
    assert lhs == rhs, "distributivity fails for polynomials: {} =/= {}".format(
...
def test_zerofier():
    ...
    for trial in range(0, 100):
        ...
        assert (
            zerofier.degree() == degree
        ), "zerofier has degree unequal to size of domain"

        for d in domain:
            assert (
                zerofier.evaluate(d) == field.zero()
            ), "zerofier does not evaluate to zero where it should"
        ...
        assert (
            zerofier.evaluate(random) != field.zero()
